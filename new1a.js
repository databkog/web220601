        ...
        $('#modal-body-space').html(content);
         ...
         $.post(`${UI.api_url}${searchhost}:id2path`, {
             ...
             if (data) {
                 ...
                 title = `Truy cập`;
                 $('#SearchModelLabel').html(title);
                 content = `<a class="btn btn-info" href="${ehrefurl}">Open</a> <a class="btn btn-secondary" href="${ehrefurl}" target="_blank">Mở trong Tab mới</a>`;
                 $('#modal-body-space').html(content);
                 return;
             }
             title = `Failed`;
             $('#SearchModelLabel').html(title);
             content = `Bị lỗi khi get link File/Folder, đóng và thử lại.`;
             $('#modal-body-space').html(content);
         })
       });
}
